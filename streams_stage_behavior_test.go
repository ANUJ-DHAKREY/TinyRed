...
const defaultMaxStreamsStage = 0

func maxStreamsStage() int {
	raw := strings.TrimSpace(os.Getenv("TINYRED_STREAMS_STAGE"))
	if raw == "" {
		return defaultMaxStreamsStage
	}
...
func requireStreamsStage(t *testing.T, stage int) {
	t.Helper()
	if stage > maxStreamsStage() {
		t.Skipf("skipping streams stage %d test; set TINYRED_STREAMS_STAGE=%d (or higher) to run", stage, stage)
	}
}
